<template>
  <div class="input-manager">
    ...
    <div class="input-manager-table">
      <el-table :data="lists" style="width: 100%">
        ...
        <el-table-column label="操作" width="300" align="center" >
          <template slot-scope="{row}">
            <el-button size="small" type="primary" @click="edit(row)" :disabled="row.status!=0&&row.status!=3&&row.status!=6">编辑</el-button>
            <el-button size="small" type="danger"  @click="del(row)" :disabled="row.status!=0&&row.status!=3&&row.status!=6">删除</el-button>
            <el-button size="small" type="success" @click="SubmitToApprove(row)" :disabled="row.status!=0&&row.status!=3&&row.status!=6">提交审核</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <!-- 编辑弹层 -->
    <el-dialog title="申请管理-编辑" :visible.sync="dialogEditVisible" center width="40%">
      <el-form :model="form">
        <el-form-item label="id" :label-width="formLabelWidth" v-show="false">
          <el-input v-model="form.id" ></el-input>
        </el-form-item>
        <el-form-item label="姓名" :label-width="formLabelWidth">
          <el-input v-model="form.name" class="filter-item" autocomplete="off"></el-input>
        </el-form-item>
